...
// ===============================
// ADMIN SECURITY - DOUBLE LAYER
// ===============================
const ADMIN_USERNAME = process.env.ADMIN_USERNAME
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD

function adminHeaderCheck(req, res, next){
...
// Apply header check to ALL admin routes
router.use(adminHeaderCheck)
...
router.get("/stats", auth, adminOnly, async(req,res)=>{
  try{
    ...
    res.json({
      ...
      pendingPayments,
      approvedPayments,
      apiRequests: requests[0]?.total || 0
    });
  }
  catch(error){
...
// APPROVE PAYMENT
// ===============================
router.post("/payments/:id/approve", auth, adminOnly, async(req,res)=>{
  try{
    ...
    if(emailService){
      await emailService.paymentApproved(user.email, payment.plan);
    }
...
// REJECT PAYMENT
// ===============================
router.post("/payments/:id/reject", auth, adminOnly, async(req,res)=>{
  try{
    const payment = await Payment.findById(req.params.id);
    if(!payment) return res.status(404).json({error:"Payment not found"});

    payment.status="rejected";
    payment.reason = req.body.reason || "Rejected";
    await payment.save();
